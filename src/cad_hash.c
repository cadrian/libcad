...
static cad_hash_key_t hash(const char *key, cad_hash_keys_t keys, int salt) {
     cad_hash_key_t result = { key, keys.hash(key) + salt };
     return result;
}
...
static hash_salt_fn hash_salt = default_hash_salt;
